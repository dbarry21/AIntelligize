...
- Used triple-class selector `h2.text-center.myls-faq-heading` with `!important`
  to beat an inline Elementor global rule (`h2.text-center { color: var(--e-global-color-primary) }`)
  that was overriding the heading colour on the frontend.
...
- **Site-wide color fallback:** The shortcode now falls back to the admin-saved
  ...
  Both shortcodes now share the same color theme set in Admin → Schema → FAQ.

**Files changed:** `service-faq-page.php`, `shortcode-data.php`,
`aintelligize.php`, `readme.txt`, `CHANGELOG.md`
