...
- **VideoObject Auto-Detector** *(v7.8.74)*: no UI — auto-runs on every singular page.
  ...
  - **Description sanitization** *(v7.9.18.89)*: Phone numbers, URLs, and connector words stripped from YouTube API descriptions before caching and 500-char truncation.
  - **YouTube Video URL metabox** *(v7.9.18.98)*: Registers on both `page` and `service` CPT. Meta: `_myls_page_video_url`. ACF `video_url` migration notice shown when legacy data exists. One-time bulk migration on version bump copies ACF values to native key. Shortcode `[myls_youtube_embed use_page_video="1"]` reads only native key.
  - **Dynamic shortcode resolution** *(v7.9.18.99, v7.9.18.100)*: Both the Elementor widget scanner and raw post_content scanner now resolve `use_page_video="1"` at schema build time, mirroring the shortcode's priority chain: `_myls_page_video_url` → `fallback_id` → site default. `$add()` de-duplication prevents duplicate VideoObject nodes.
  - **One-video cap** *(v7.9.18.101)*: Service CPT pages capped to one VideoObject — page-specific video takes priority over template fallback. Bypassable via `myls_service_video_cap` filter.
