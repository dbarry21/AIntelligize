...
- **VideoObject Auto-Detector** *(v7.8.74)*: no UI — auto-runs on every singular page.
  ...
  - **Dynamic shortcode resolution** *(v7.9.18.99)*: Elementor shortcode scanner now resolves `use_page_video="1"` at schema build time, mirroring the shortcode's priority chain: `_myls_page_video_url` → `fallback_id` → site default. Mutually exclusive with `video_id=` to prevent duplicate VideoObject nodes.
