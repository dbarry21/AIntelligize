		...
		foreach ( $elements as $el ) {
			...
			if ( $widget_type === 'shortcode' ) {
				...
				if ( $sc_content !== '' ) {
					...
					// Case 1: hardcoded video_id= attribute — direct match.
					if ( preg_match_all(
					...
					// Case 2: use_page_video="1" — mirror the shortcode resolution chain.
					// Replicates youtube-embed.php priority order at schema build time.
					} elseif ( preg_match(
						...
						// 1) Per-page meta — plugin-native key, highest priority.
						...
						if ( $resolved_id === '' ) {
							$default_id = trim( (string) get_option( 'myls_ytvb_default_video_id', '' ) );
							if ( $default_id !== '' ) {
								$resolved_id = preg_replace( '/[^A-Za-z0-9_\-]/', '', $default_id );
							}
						}

						// Emit VideoObject only when a valid 11-char ID was resolved.
						if ( $resolved_id !== '' && strlen( $resolved_id ) === 11 ) {
							$item = myls_make_video_item(
								'https://www.youtube.com/watch?v=' . rawurlencode( $resolved_id ),
								'', '', '', $post_id
							);
							if ( $item ) $found[] = $item;
						}
